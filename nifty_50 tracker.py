# app.py — NIFTY50 Dashboard (Latest production-ready)
# - Yahoo -> AlphaVantage -> NSE fallback
# - Robust history fallback (yfinance + Alpha)
# - Polite Alpha rate-limiter + retries
# - Sanitize for Streamlit / duplicate columns handling
# - sector override support via sectors_override.csv
# - dev_mode with verbose diagnostics
# - No async / no aiohttp
...
# Config & constants
# --------------------------
st.set_page_config(page_title="NIFTY50 — Latest Dashboard", layout="wide")
st.title("📈 NIFTY50 — Latest Dashboard")
...
SECTOR_OVERRIDE_PATH = "sectors_override.csv"

# Developer options
dev_mode = st.sidebar.checkbox("Dev mode (verbose diagnostics)", value=False)
max_alpha_calls_per_min = st.sidebar.number_input("AlphaVantage calls/min (<=5 recommended)", min_value=1, max_value=20, value=5)
...
def sanitize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if isinstance(series, pd.DataFrame):
            for i, sub in enumerate(series.columns):
                cleaned[f"{col}_{i}"] = series[sub].apply(_cell)
        else:
            cleaned[col] = series.apply(_cell)
    return pd.DataFrame(cleaned)

# --------------------------
# Simple Alpha rate limiter + retry helper
# --------------------------
class AlphaLimiter:
...
alpha_limiter = AlphaLimiter(max_alpha_calls_per_min)
...
# Sector override
# --------------------------
def load_sector_override(path=SECTOR_OVERRIDE_PATH) -> Dict[str, str]:
...
def load_live(tickers):
    rows = []
    try:
        bulk_prices = yf.download(tickers=tickers, period="2d", interval="1d", group_by="ticker", threads=True, progress=False, auto_adjust=False)
    except Exception:
    ...
    for t in tickers:
        sym = t.replace(".NS","")
        company = sym
        price = None; prev = None; pct = None; mcap = None; sector = None; source = None

        # yfinance bulk
        try:
            if isinstance(bulk_prices, pd.DataFrame) and t in bulk_prices.columns.get_level_values(0):
                closes = bulk_prices[t]["Close"].dropna()
                if len(closes) >= 1:
        ...
        # alpha overview for missing sector/mcap
...
if df is None or df.empty:
    st.error("Data load failed. Check network and API keys.")
    st.stop()
...
# market cap calc for weighting (do not surface as real value)
...
min_mcap = st.sidebar.number_input("Min market cap (crore INR)", value=0.0, step=100.0)
...
if min_mcap > 0:
    display_df = display_df[display_df["Market Cap Calc"] >= (min_mcap * 1e7)]

# top metrics
pos = display_df.loc[display_df["% Change"]>0,"% Change"].sum()
neg = display_df.loc[display_df["% Change"]<0,"% Change"].sum()
impact = display_df["Weighted Impact"].sum()
c1,c2,c3 = st.columns(3)
c1.metric("Total Gainers Impact", f"{pos:.2f} %")
c2.metric("Total Losers Impact", f"{neg:.2f} %")
c3.metric("Overall Weighted Impact", f"{impact:.2f} %")
st.write("---")
...
if len(sector_impact) <= 1:
    st.warning("Sector metadata limited. Please provide ALPHAVANTAGE_API_KEY in Streamlit secrets or a sectors_override.csv.")
else:
    st.subheader("Weighted Sector Impact")
    st.plotly_chart(px.bar(sector_impact.sort_values("Weighted Impact"), x="Sector", y="Weighted Impact", color="Weighted Impact", color_continuous_scale=px.colors.diverging.RdYlGn), use_container_width=True)
...
if trend_count["Count"].sum() == 0:
    ...
    st.plotly_chart(px.pie(trend_count, names="Trend", values="Count", color_discrete_map={"Gainer":"green","Loser":"red"}, title="Gainers vs Losers"), use_container_width=True)
...
sel = df[df["Ticker"]==choice].iloc[0] if choice else None
if sel is not None:
    st.markdown(f"**{sel['Company']}** ({sel['Ticker']}) — Sector: {sel['Sector'] if pd.notna(sel['Sector']) else 'Unknown'}")
    st.markdown(f"Price: **{sel['Price'] if pd.notna(sel['Price']) else 'N/A'}** | Prev: **{sel['Prev Close'] if pd.notna(sel['Prev Close']) else 'N/A'}** | %: **{sel['% Change']:.2f}%**" if pd.notna(sel['% Change']) else "N/A")
    st.markdown(f"Market Cap: **{format_mcap(sel['Market Cap'])}** — Stake: **{sel['Stake (%)']:.2f}%** — Source: {sel['Source']}")
    try:
        hist90 = get_history_for_ticker(sel["Ticker"], datetime.now().date()-timedelta(days=90), datetime.now().date())
        if hist90 is not None and not hist90.empty:
            st.plotly_chart(px.line(hist90, y="Close", title=f"{sel['Ticker']} — Last 90 days"), use_container_width=True)
            st.download_button("Download 90d CSV", data=hist90.to_csv().encode(), file_name=f"{sel['Ticker']}_90d.csv", mime="text/csv")
        else:
...
# diagnostics
if dev_mode:
    with st.expander("Diagnostics (data sources & missing fields)"):
    ...
    st.write("Alpha limiter interval (s):", alpha_limiter.interval)

st.caption("Latest build — set ALPHAVANTAGE_API_KEY in Streamlit secrets for best metadata. Add sectors_override.csv as fallback if you prefer local overrides.")
